...
APP_TITLE = "Math Stats"
START_PAGE = "HTML/Welcome.html"
DEFAULT_PORT = 8765


def base_dir() -> Path:
    # In one-file PyInstaller mode, resources are unpacked to _MEIPASS.
...
def main():
    ...
    try:
        webview.create_window(APP_TITLE, start_url, width=1280, height=800)
        webview.start()
    finally:
        server.shutdown()
        server.server_close()
